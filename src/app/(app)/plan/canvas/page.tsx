...
const LIFE_AREA_CONFIGS: Record<string, LifeAreaConfig> = {
  study: {
    ...
    colorBorder: "border-amber-500/20 dark:border-amber-500/30",
    colorText: "text-amber-700 dark:text-amber-400",
    barBg: "bg-gradient-to-r from-amber-500/15 via-amber-500/10 to-amber-500/25 dark:from-amber-500/25 dark:via-amber-500/15 dark:to-amber-500/30",
    barBorder: "border-amber-400/50 dark:border-amber-500/40",
    barRing: "#f59e0b",
  ...
  work: {
    ...
    colorBorder: "border-blue-500/20 dark:border-blue-500/30",
    colorText: "text-blue-700 dark:text-blue-400",
    barBg: "bg-gradient-to-r from-blue-500/15 via-blue-500/10 to-blue-500/25 dark:from-blue-500/25 dark:via-blue-500/15 dark:to-blue-500/30",
    barBorder: "border-blue-400/50 dark:border-blue-500/40",
    barRing: "#3b82f6",
  ...
  health: {
    ...
    colorBorder: "border-emerald-500/20 dark:border-emerald-500/30",
    colorText: "text-emerald-700 dark:text-emerald-400",
    barBg: "bg-gradient-to-r from-emerald-500/15 via-emerald-500/10 to-emerald-500/25 dark:from-emerald-500/25 dark:via-emerald-500/15 dark:to-emerald-500/30",
    barBorder: "border-emerald-400/50 dark:border-emerald-500/40",
    barRing: "#10b981",
  ...
  personal: {
    ...
    colorBorder: "border-rose-500/20 dark:border-rose-500/30",
    colorText: "text-rose-700 dark:text-rose-400",
    barBg: "bg-gradient-to-r from-rose-500/15 via-rose-500/10 to-rose-500/25 dark:from-rose-500/25 dark:via-rose-500/15 dark:to-rose-500/30",
    barBorder: "border-rose-400/50 dark:border-rose-500/40",
    barRing: "#f43f5e",
  ...
  finance: {
    ...
    colorBorder: "border-purple-500/20 dark:border-purple-500/30",
    colorText: "text-purple-700 dark:text-purple-400",
    barBg: "bg-gradient-to-r from-purple-500/15 via-purple-500/10 to-purple-500/25 dark:from-purple-500/25 dark:via-purple-500/15 dark:to-purple-500/30",
    barBorder: "border-purple-400/50 dark:border-purple-500/40",
    barRing: "#a855f7",
  ...
  project: {
    ...
    colorBorder: "border-indigo-500/20 dark:border-indigo-500/30",
    colorText: "text-indigo-700 dark:text-indigo-400",
    barBg: "bg-gradient-to-r from-indigo-500/15 via-indigo-500/10 to-indigo-500/25 dark:from-indigo-500/25 dark:via-indigo-500/15 dark:to-indigo-500/30",
    barBorder: "border-indigo-400/50 dark:border-indigo-500/40",
    barRing: "#6366f1",
...
function getAreaConfig(areaName: string | null): LifeAreaConfig {
  ...
  return (
    LIFE_AREA_CONFIGS[key] || {
      ...
      icon: Target,
      colorBg: "bg-slate-500/10 dark:bg-zinc-500/20",
      colorBorder: "border-slate-500/20 dark:border-zinc-500/30",
      colorText: "text-slate-700 dark:text-zinc-300",
      barBg: "bg-gradient-to-r from-slate-500/15 via-slate-500/10 to-slate-500/25 dark:from-zinc-500/25 dark:via-zinc-500/15 dark:to-zinc-500/30",
      barBorder: "border-slate-400/50 dark:border-zinc-500/40",
      barRing: "#64748b",
...
function ProgressRing({
  ...
  return (
    <div className="relative inline-flex items-center justify-center shrink-0" style={{ width: size, height: size }}>
      ...
      <span className="absolute text-[9px] font-bold text-slate-700 dark:text-zinc-300 font-mono">
        {clampedPercent}
      </span>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-8 pb-16">
      ...
      <header className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-slate-500 dark:text-zinc-400 text-xs font-semibold uppercase tracking-wider mb-1">
        ...
        <div className="flex items-center gap-3 self-start lg:self-auto flex-wrap">
          {/* View Toggle */}
          <div className="inline-flex items-center p-1 bg-slate-100 dark:bg-zinc-800/80 rounded-xl border border-slate-200 dark:border-zinc-700/60 shadow-2xs">
            <Link
              href="/plan/goals"
              className="px-3.5 py-1.5 text-xs font-semibold rounded-lg text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-100 transition-colors flex items-center gap-1.5"
            >
            ...
            <Link
              href="/plan/canvas"
              className="px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-white dark:bg-zinc-900 text-slate-900 dark:text-zinc-100 shadow-xs flex items-center gap-1.5 border border-slate-200/50 dark:border-zinc-700/50"
            >
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-100/80 dark:border-indigo-800/50 text-indigo-700 dark:text-indigo-300 text-xs font-semibold rounded-lg">
      ...
      <section className="bg-white dark:bg-zinc-900 border border-zinc-200/70 dark:border-zinc-800/60 rounded-2xl p-4 shadow-subtle flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs text-zinc-600 dark:text-zinc-400">
          <span className="font-medium uppercase tracking-wider text-zinc-400 dark:text-zinc-500 text-[10px]">
            Timeline Window:
          </span>
        ...
        <div className="overflow-x-auto min-w-full">
          <div className="min-w-[1280px] relative pb-6">
            ...
            <div className="sticky top-0 z-20 bg-white/85 dark:bg-zinc-900/85 backdrop-blur-xl border-b border-zinc-200/70 dark:border-zinc-800/60">
                ...
                {quarters.map((q, idx) => (
                  <div
                    ...
                    className={`py-2.5 px-3 text-center border-r border-zinc-200/60 dark:border-zinc-800/60 last:border-r-0 ${
                      idx % 2 === 0 ? "bg-zinc-50/40 dark:bg-zinc-800/30" : "bg-transparent"
                    }`}
                  >
              ...
              {/* Months Row */}
              <div className="grid grid-cols-24 text-[10px] text-zinc-400 dark:text-zinc-500 font-mono">
                {quarters.flatMap((q) =>
                  q.months.map((m, mIdx) => (
                    <div
                      key={`${q.label}-${mIdx}`}
            ...
            <div className="divide-y divide-slate-100 dark:divide-zinc-800/60">
              {displayAreas.map(([areaKey, areaGoals]) => {
                ...
                return (
                  <div
                    key={areaKey}
                    className="group relative hover:bg-slate-50/40 dark:hover:bg-zinc-800/30 transition-colors py-4 px-4"
                  >
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[11px] font-semibold text-slate-400 dark:text-zinc-400 px-2 py-0.5 bg-slate-100 dark:bg-zinc-800 rounded-md border border-slate-200/60 dark:border-zinc-700/60">
                    ...
                    <div className="relative min-h-[60px] flex flex-col gap-2.5">
                      ...
                      <div className="absolute inset-0 grid grid-cols-24 pointer-events-none opacity-40">
                        {Array.from({ length: 24 }).map((_, colIdx) => (
                          <div
                            key={colIdx}
                            className={`border-r border-slate-200/50 dark:border-zinc-800/50 ${
                              colIdx % 3 === 2 ? "border-r-slate-300 dark:border-r-zinc-700" : ""
                            }`}
                          />
                        ))}
                      </div>
                      ...
                      {areaGoals.length > 0 ? (
                            ...
                            >
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2 min-w-0">
                                  ...
                                  <div className="min-w-0">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      {isPaused && (
                                        <span className="inline-flex items-center gap-0.5 text-[9px] font-bold px-1.5 py-0.2 rounded-md bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800/50">
                                          <PauseCircle className="w-2.5 h-2.5 text-amber-600 dark:text-amber-400" />
                                          Paused
                                        </span>
                                      )}
                                    </div>
                                    <div className="text-[10px] text-slate-500 dark:text-zinc-400 font-medium truncate flex items-center gap-1">
                                ...
                                <Link
                                  ...
                                  className="opacity-0 group-hover/card:opacity-100 transition-opacity p-1 bg-white/90 dark:bg-zinc-800/90 rounded-md border border-slate-200/80 dark:border-zinc-700/80 text-slate-600 dark:text-zinc-300 hover:text-indigo-600 dark:hover:text-indigo-400 shadow-2xs shrink-0"
                                  title="View Roadmaps"
                                >
                        ...
                        <div className="h-10 flex items-center text-xs text-slate-400 dark:text-zinc-500 italic pl-2">
      ...
      <footer className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500 dark:text-zinc-400 bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-4">
