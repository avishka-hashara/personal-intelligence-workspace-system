...
export function DataImportManager({ coursesList }: DataImportManagerProps) {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-16">
      ...
      {/* Header Description */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-zinc-100 tracking-tight">Data Importers</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {/* =================================================================== */}
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xs p-6 flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-red-50 dark:bg-rose-950/40 text-red-600 dark:text-rose-400 flex items-center justify-center border border-red-100 dark:border-rose-900/50">
                <CheckSquare className="w-5 h-5" />
              </div>
              <div>
                <h2 className="text-base font-bold text-slate-900 dark:text-zinc-100">Import Tasks (Todoist CSV)</h2>
                <span className="text-xs text-slate-500 dark:text-zinc-400 font-medium">Standard Todoist Export</span>
              </div>
            </div>

            <p className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed">
              Upload your Todoist task export. We map the <code className="bg-slate-100 dark:bg-zinc-800 px-1 py-0.5 rounded text-[11px] font-semibold text-slate-800 dark:text-zinc-200">CONTENT</code> column to task titles, <code className="bg-slate-100 dark:bg-zinc-800 px-1 py-0.5 rounded text-[11px] font-semibold text-slate-800 dark:text-zinc-200">DUE DATE</code> to schedules, and maintain fractional sorting.
            </p>

            <form onSubmit={handleTodoistSubmit} className="space-y-4 pt-2">
              {/* File Dropzone / Picker */}
              <div className="relative border-2 border-dashed border-slate-200 dark:border-zinc-700 hover:border-indigo-400 dark:hover:border-indigo-500 rounded-xl p-5 text-center transition-colors bg-slate-50/50 dark:bg-zinc-800/40">
                <input
                  type="file"
                ...
                <div className="flex flex-col items-center gap-2 pointer-events-none">
                  ...
                  {todoistFile ? (
                    <div className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 truncate max-w-xs">
                    ...
                    <>
                      ...
                      <p className="text-[11px] text-slate-400 dark:text-zinc-500">Accepts .csv files exported from Todoist</p>
                    </>
                  )}
                </div>
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-900 font-medium text-xs shadow-xs disabled:opacity-50 transition-all cursor-pointer"
              >
                {isTodoistPending ? (
                  <>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-zinc-800 flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-zinc-500">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xs p-6 flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-900 dark:text-zinc-100">Import Flashcards (Anki TSV)</h2>
                <span className="text-xs text-slate-500 dark:text-zinc-400 font-medium">FSRS Memory Engine Compatible</span>
              </div>
            </div>

            <p className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed">
              Upload a tab-separated text/TSV file exported from Anki (<code className="bg-slate-100 dark:bg-zinc-800 px-1 py-0.5 rounded text-[11px] font-semibold text-slate-800 dark:text-zinc-200">Front \t Back</code>). Cards will be scheduled using the FSRS spaced repetition engine.
            </p>

            <form onSubmit={handleAnkiSubmit} className="space-y-4 pt-2">
              {/* Course Selector */}
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-slate-700 dark:text-zinc-300">Target Course</label>
                {coursesList.length === 0 ? (
                  <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 text-xs border border-amber-200 dark:border-amber-800 flex items-center justify-between">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-xs font-medium text-slate-800 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                  >
                    {coursesList.map((c) => (
                      <option key={c.id} value={c.id} className="bg-white dark:bg-zinc-800 text-slate-800 dark:text-zinc-200">
                        {c.code}: {c.title}
                      </option>
                    ))}
                  </select>
                ...
                <div className="flex flex-col items-center gap-2 pointer-events-none">
                  ...
                  {ankiFile ? (
                    ...
                    <>
                      ...
                      <p className="text-[11px] text-slate-400 dark:text-zinc-500">Accepts Anki export cards (tab-separated)</p>
                    </>
                  )}
                </div>
              </div>
      ...
      {/* =================================================================== */}
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-rose-200 dark:border-rose-900/60 p-6 shadow-xs space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <Trash2 className="w-5 h-5 text-rose-600 dark:text-rose-400" />
              <h2 className="text-base font-bold text-rose-950 dark:text-rose-200">Delete Account & Data</h2>
            </div>
  ...
  return (
    <>
      <button
        ...
        className="px-4 py-2 rounded-xl bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/50 text-rose-700 dark:text-rose-300 font-semibold text-xs border border-rose-200 dark:border-rose-900/60 transition-colors cursor-pointer"
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-zinc-800 space-y-4 animate-in fade-in zoom-in-95 duration-150 text-slate-900 dark:text-zinc-100">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-rose-100 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400 flex items-center justify-center">
                <Trash2 className="w-5 h-5" />
              </div>
              <div>
                <h3 className="font-bold text-slate-900 dark:text-zinc-100 text-base">Confirm Account Deletion</h3>
                <span className="text-xs text-slate-500 dark:text-zinc-400">7-Day Cancellation Grace Period</span>
              </div>
            </div>
            ...
            <form onSubmit={handleDelete} className="space-y-4 pt-2">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-zinc-700 text-xs text-slate-900 dark:text-zinc-100 bg-transparent dark:bg-zinc-800/60 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
                />
              </div>
